...
| `FUNCTIONS_WORKER_RUNTIME_VERSION` | `24` (Ubuntu 24.04 Noble) |
...
Users select a runtime that describes what is actually in the image: an OS with the Functions Host and nothing else. The "version" is the OS major release number (e.g., `24` for Ubuntu 24.04 Noble), which unambiguously refers to the base operating system, not a language version that is not present. Compare:

| Today (per-language) | Proposed (native) |
|---|---|
| `--runtime golang --runtime-version 1.0` | `--runtime native --runtime-version 24` |
| *"Is 1.0 the Go version?"* | *"The image ships Ubuntu 24.04 and the host. I bring my binary."* |
...
ENV FUNCTIONS_WORKER_RUNTIME=native \
    FUNCTIONS_WORKER_RUNTIME_VERSION=24
```
...
| Version semantics | OS version (`al2023`) | OS major version (`24`) |

## What Changes

| Component | Change Required |
|---|---|
| **Docker images** | Add `native24.Dockerfile` for Flex Consumption and Linux Dedicated (2 files, ~15 lines each, Noble only) |
| **Pipeline config** | Add `native24` identifier to `build_validate_publish.yml` |
...
A `native` runtime replaces per-language images for compiled languages with a single, reusable image. It provides clear version semantics tied to the OS major version, eliminates operational overhead for new languages, and follows the same pattern that AWS Lambda has validated at scale with `provided` runtimes. The worker config is written inline in the Dockerfile to allow updates without waiting for host releases.
